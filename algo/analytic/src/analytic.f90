...
SUBROUTINE analytic_coeff(model_coul, thres, freq, coul, coul_coeff)
  ...
  !> the screened Coulomb interaction on the frequency grid
  COMPLEX(dp), INTENT(IN) :: coul(:,:,:)

  !> the coefficients used to evaluate the screened Coulomb interaction
  !! at an arbitrary frequency
  COMPLEX(dp), INTENT(OUT), ALLOCATABLE :: coul_coeff(:,:,:)
  ...
  !!    the parameters.
  CASE (godby_needs)
    CALL godby_needs_coeffs(AIMAG(freq%solver(2)), coul_coeff)
  ...
  CASE (pade_robust) 
    CALL pade_coeff_robust(freq%solver, thres, coul_coeff)
    ...
    DO igp = 1, num_g_corr
      DO ig = 1, num_g_corr
        ...
        IF (model_coul == aaa_approx) THEN
          ...
          ! store the coefficients in the same array
          coul_coeff(ig, igp, :) = zero
          coul_coeff(ig, igp, 0 * mmax + 1 : 0 * mmax + mm) = aaa%position
          coul_coeff(ig, igp, 1 * mmax + 1 : 1 * mmax + mm) = aaa%value
          coul_coeff(ig, igp, 2 * mmax + 1 : 2 * mmax + mm) = aaa%weight

        ELSE
          ! correct poles of AAA approximation
          CALL pole_correction(thres, aaa, coul_coeff(ig, igp, :))
        END IF
...
SUBROUTINE analytic_eval(gmapsym, grid, freq_in, coul_coeff, freq_out, coul)
  ...
  COMPLEX(dp),              INTENT(IN)  :: coul_coeff(:,:,:)
  ...
  COMPLEX(dp), ALLOCATABLE, INTENT(OUT) :: coul(:,:)
  ...
  !
  ALLOCATE(coul(grid%corr_fft%nnr, grid%corr_par_fft%nnr), STAT = ierr)
  IF (ierr /= 0) THEN
  ...
  DO igp = 1, grid%corr_par_fft%ngm
    ...
    DO ig = 1, grid%corr_fft%ngm
      ...
      coeff = coul_coeff(gmapsym(ig), gmapsym(igp_g), :)
      ...
      CASE (pade_approx)
        ...
        CALL pade_eval(freq_in%num_freq(), freq, coeff, freq_sym, coul(ig, igp))

      CASE (pade_robust)
        !
        ! robust Pade analytic continuation
        CALL pade_eval_robust(coeff, freq_sym, coul(ig, igp))

      CASE (godby_needs)
        !
        ! Godby-Needs Pole model
        coul(ig, igp) = godby_needs_model(freq_sym, coeff)

      CASE (aaa_approx)
        !
        ! AAA approximation
        coul(ig, igp) = aaa_approx_eval(freq_sym, coeff)

      CASE (aaa_pole)
        !
        ! AAA approximation with pole removal
        coul(ig, igp) = aaa_pole_eval(freq_sym, coeff)
